...
## S2-25 Implementation Note

S2-25 implements the minimal authenticated `/reports/history` frontend page described in Section 7. The optional regenerate route remains design-only; cloud storage adapters, signed URLs, and queue-backed generation also remain future work.

- Implemented route: `GET /reports/history` (React Router, gated by `authed`). Mounted in `apps/web/src/App.jsx`.
- New nav entry: `AppShell` shows a `Reports` link (sits between `Reviews` and `Members`). The label is `Reports` because the existing nav has no other reports item; a `Report definitions` page can split the labeling later if needed.
- Implemented page: `apps/web/src/pages/ReportHistory.jsx`. Inside the existing `AppShell` it shows an organization picker (uses the existing `GET /api/v1/orgs` endpoint via `listOrganizationsForReports`), a labeled filter form (`status`, `report_type`, `report_key`, `date_from`, `date_to`, `limit`), and a runs list. Per-run cards show `report_name`/`report_key`/`report_type`, a status badge, scope summary (`organization`/`client`/`location` ids only — never emails), `requested_by_user_id`, `created_at`/`completed_at`, and one row per output with format, status, size, storage provider, and a `Download <FORMAT>` button when the output is `succeeded` with `storage_provider` and `storage_key`. The page never displays `storage_key`, absolute paths, base64 bodies, or emails.
- Implemented helpers: `apps/web/src/lib/reportHistory.js` exports `REPORT_RUN_STATUSES`, `REPORT_RUN_FORMATS`, `REPORT_LIST_LIMIT_DEFAULT`/`_MIN`/`_MAX`, `clampReportListLimit`, `buildReportRunsQuery`, `parseContentDispositionFilename`, `safeDownloadFilename` (mirrors the backend `^[A-Za-z0-9._-]+$` allow-list), `formatBytes`, `normalizeReportRunRow` (drops `storage_key` defensively and exposes `downloadable`), `describeReportHistoryError`, `listOrganizationsForReports`, `listReportRunsForUser` (throws when `organization_id` is missing), and `downloadReportOutput` (calls the raw-bytes download endpoint via `fetch` because the shared JSON-aware `api()` client cannot return binary bodies). 28 Vitest tests in `apps/web/src/lib/reportHistory.test.js` cover constants, limit clamping, query building, filename parsing/sanitization, byte formatting, error formatting, row normalization (including a `JSON.stringify` scan that confirms `storage_key` is never exposed), and the download helper (URL/header construction with an injected fake fetch).
- Download flow: per-output button → `downloadReportOutput({ runId, format })` → `downloadBlob(filename, blob)` (the existing helper in `apps/web/src/reportDownloads.js`, which creates an object URL and triggers a click). The filename comes from the server's `Content-Disposition` (sanitized through `safeDownloadFilename`); the page never constructs storage URLs and never reads `storage_key`. On non-2xx responses the helper throws the parsed `{ code, message, status }` envelope and the page surfaces it inline without clearing app auth.
- Auth/error behavior: a real app-auth `401` continues to follow the existing `api()` client redirect-to-login behavior. `403`/`404`/`409`/`500` envelopes display inline. The Google provider reauth banner is not touched by the new page or helpers.
...
S2-29 adds dedicated rate-limit buckets and audit events for the read-only report listing and download routes designed in S2-20 Section 3 and implemented in S2-23 / S2-24. Generation (`POST /api/v1/reports/dashboard-snapshot`) remains on its existing `generation` bucket and its existing `report.dashboard_snapshot.generate` audit event. No listing/download API contract field changed; response shapes and error codes are identical to S2-23 / S2-24. The listing/download routes now also accept the standard `429 rate_limited` envelope from the shared rate-limit middleware when a bucket is exceeded.

- Rate-limit buckets (defaults shared with the existing `RATE_LIMIT_WINDOW_SECONDS` window; default `600s`):
  - `report_list` — applied to `GET /api/v1/reports/runs`. Default cap `120` per window per `req.user.user_id` (falls back to IP per `getClientIdentity`). Env override: `RATE_LIMIT_REPORT_LIST_MAX`.
  - `report_download` — applied to `GET /api/v1/reports/runs/:runId/outputs/:format`. Default cap `60` per window per `req.user.user_id` (falls back to IP). Env override: `RATE_LIMIT_REPORT_DOWNLOAD_MAX`.
  - Both reuse the existing shared in-process store and emit the same `{ error: { code: "rate_limited", message, retry_after_seconds } }` body plus `Retry-After` / `X-RateLimit-*` headers. Distributed (Redis-backed) rate limiting remains a separate Phase 0 hardening follow-up.
- Audit events (best-effort; failures swallowed by `writeAuditLog` so the user request never fails):
  - `report.run.list` (success) — compact metadata `{ report_type?, report_key?, status?, date_from?, date_to?, limit, result_count, has_more, membership_role }` plus `target_type: "report_run"`, `organization_id`, optional `client_id`, optional `location_id`. Filter strings only appear when supplied by the requester; raw `report_runs` rows are never logged.
  - `report.run.list_failed` (failure) — same scope fields plus `{ ...filters, limit, reason: { code, message }, status }`. Reason is the compact `{ code, message }` shape used elsewhere in the route; the original error payload is not logged.
  - `report.output.download` (success) — `{ report_run_id, format, size, content_type, storage_provider, checksum_algorithm, membership_role }` plus `target_type: "report_run_output"`, `target_id: <run_id>`, `organization_id`. The audit never logs `storage_key`, the filename, the bytes, absolute paths, or base64 payloads.
  - `report.output.download_failed` (failure) — `{ report_run_id, format?, reason: { code, message }, status }` plus `target_type: "report_run_output"`, `target_id: <run_id>`. `format` is normalized to lowercase `pdf`/`xlsx` and is `null` when the requester supplied an unsupported value (preventing arbitrary strings from leaking into audit metadata).
- The audit payloads are computed by the four pure helpers `buildListAuditDetails(query, result)`, `buildListFailureAuditDetails(query, error)`, `buildDownloadAuditDetails({ runId, format, result })`, and `buildDownloadFailureAuditDetails({ runId, format, error })` exported from `apps/api/src/routes/reports.js` for direct unit testing. The route handlers stay thin and just call `auditSuccess` / `auditFailure` with the produced payload.
- Sanitization guarantees: every audit payload is additionally passed through the existing `sanitizeAuditMetadata` pipeline (depth/length caps, secret-key redaction) before being persisted. No JWTs, OAuth tokens, refresh tokens, ID tokens, auth codes, authorization headers, encrypted secret payloads, passwords, emails, raw user records, storage roots, absolute paths, raw snapshots, raw buffers, base64, or raw Mongo documents are written to audit metadata by any of the new events.
- Proof: `docs/proof/s2-29-report-audit-rate-limit-hardening.md`. No frontend file changed. No new dependency installed; `package-lock.json` unchanged.
...
S2-25.1 ran a local API + web dev-server smoke against the controlled `s2-15-fixture-org` scope to verify the S2-25 page end-to-end. Proof is recorded in `docs/proof/s2-25-1-report-history-ui-browser-smoke.md`. The smoke confirmed: `GET /api/v1/health` returned `200`; the Vite dev server returned the SPA shell with `200` for both `/` and `/reports/history`; the dev-served React modules carry the documented route (`/reports/history` in `App.jsx`), nav (`Reports` → `/reports/history` in `AppShell.jsx`), and page wiring (`ReportHistory.jsx` imports `listReportRunsForUser`/`downloadReportOutput`/`formatBytes`/`REPORT_RUN_STATUSES`, renders the documented `/tmp` warning copy, and contains zero `storage_key` references); `GET /api/v1/reports/runs?organization_id=s2-15-fixture-org` returned both fixture rows (S2-22.1 + S2-24.1) with the documented `{ pagination: { limit, has_more, next_cursor: null } }` shape; the documented filter combo (`status=succeeded&report_type=dashboard_snapshot&report_key=s2-24-1-smoke-dashboard&date_from=2026-05-15&date_to=2026-05-17&limit=1`) narrowed to the S2-24.1 row; `GET /api/v1/reports/runs/:runId/outputs/{pdf,xlsx}` under the owner JWT returned raw bytes (PDF=2047 / sha256 `fc62c1bb2b92...`; XLSX=8678 / sha256 `6c1331a5967c...`) with `Content-Type` from persisted `output.content_type`, ASCII-safe `Content-Disposition: attachment; filename="..."`, matching `Content-Length`, `Cache-Control: no-store`, and `X-Content-Type-Options: nosniff`; the page's `parseContentDispositionFilename` + `safeDownloadFilename` helpers were re-run against the captured headers and produced the same allow-listed filenames; the listing response contained no `_id`/`input_snapshot`/`buffer`/`base64`/`/tmp/`/`/var/www/`/`REPORT_STORAGE_LOCAL_DIR`/storage-root literal/`@`-style address leakage; and the documented denial/error envelopes were exercised non-mutatively (`400 bad_request` for listing without `organization_id` and for download with `outputs/csv`; `403 organization_role_required` for `member` listing; `403 organization_scope_required` for `manager` listing without scope and for `manager` download on the org-level run; `401 unauthorized` for the no-auth download probe). Interactive in-browser DOM mount and `<a download>` click were skipped because no headless-browser tool is available in this environment; the fetch URL/header contract the page builds was reproduced exactly. Only the API and the Vite dev server were started; workers and scheduler were not, and both processes were stopped after the smoke.

### S2-24.1 Live Smoke

S2-24.1 ran a live local API + MongoDB smoke against the controlled `s2-15-fixture-org` scope to verify the S2-24 download endpoint end-to-end. Proof is recorded in `docs/proof/s2-24-1-report-output-download-live-smoke.md`. The smoke confirmed HTTP 200 for `owner`/`admin` downloads with raw PDF/XLSX bytes (not JSON, not base64), `Content-Type` matching the persisted `output.content_type`, `Content-Disposition: attachment` with an ASCII-safe filename (`^[A-Za-z0-9._-]+$`), `Content-Length` matching the read buffer length, `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, downloaded byte size matching the persisted `output.size`, downloaded sha256 matching the persisted `output.checksum.value`, and the denial codes `organization_scope_required` (manager/viewer against an org-level run), `organization_role_required` (`member`), and `organization_membership_required` (`invited`, `disabled`). Error coverage: `400 bad_request` for invalid/unknown formats, `404 report_run_not_found` for an unknown run id, and `401 unauthorized` for the no-auth probe. The `404 report_output_not_found`, `409 report_output_not_ready`, `500 report_output_read_failed`, and `500 report_output_integrity_failed` branches were skipped live because reproducing them safely requires data mutation; they remain covered by the S2-24 unit tests. Because the original S2-22.1 storage directory under `/tmp` was wiped between 2026-05-12 and 2026-05-16, one fresh fixture run was generated under the controlled scope (new `report_key: s2-24-1-smoke-dashboard`) before downloading. Org-level coverage only; the manager/viewer positively-matching-scope download path remains covered by the S2-24 unit tests.
