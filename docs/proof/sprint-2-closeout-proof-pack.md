...
- **S2-20**: report history / listing UI design or report storage design. Choose whichever direction the product calls for next (history-first or storage-first), produce a contract-only task before implementation, and keep generation synchronous in the interim. Complete; contract recorded in `docs/architecture/report-history-and-storage.md`; proof recorded in `docs/proof/s2-20-report-history-storage-contract.md`.
