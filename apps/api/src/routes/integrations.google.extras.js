...
async function getAccessTokenForLocation(userId, loc) {
  // Prefer the integration that imported/owns this location (critical for multi-account)
  const integ = loc?.integration_id
    ? await getGoogleIntegrationById(userId, loc.integration_id)
    : await getActiveGoogleIntegration(userId); // fallback for legacy rows
...
router.get('/performance-series', authenticate, async (req, res) => {
  try {
    ...
    // --- Range handling: start/end preferred, else fallback to days ---
    ...
    function parseYmdStrict(s) {
      ...
      // Validate it didn't roll (e.g. 2026-02-31)
    ...
    if (qsStart && qsEnd) {
      ...
      // Clamp to max 366 days (inclusive)
    ...
    // Optional: prevent future end date (keeps Google API behavior sane)
...
router.put('/reviews/reply', authenticate, async (req, res) => {
  try {
    ...
    if (!locationId || !reviewName || !comment) {
      return res.status(400).json({
        error: { code: 'bad_request', message: 'locationId + reviewName + comment required' }
      });
    }
