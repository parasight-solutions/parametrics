...
import { getActiveGoogleIntegration, getGoogleIntegrationById } from "../integrations/google.store.js";
...
router.get("/", authenticate, async (req, res) => {
  try {
    ...
    if (provider === "google" && !all) {
      ...
      const list = await locations.find(q).sort({ updated_at: -1 }).project({ _id: 0 }).toArray();
